...
int main() {
    ...
    const uint64_t total_size = (1ULL << 27); // 2^27
    const std::vector<uint64_t> ratios = {1, 10000, 40000};
    ...
    for (auto ratio : ratios) {
        ...
        // --- Generate small table with replacement (duplicates allowed) ---
        ...
        for (uint64_t i = 0; i < n_small; ++i) {
            values_small[i] = dist_small(gen_small);
        }

        // --- Write to file ---
        std::string name = "./nonunique_ratio_" + std::to_string(ratio) + "to1.txt";
        ...
        std::cout << "Generated " << name << ": S=" << n_big
                  << " (unique), R=" << n_small << " (with replacement)\n";
    }
